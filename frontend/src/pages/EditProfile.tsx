...
import {ArrowLeft,Camera,Save,User as UserIcon, Mail,Lock,Eye,EyeOff,CheckCircle2,AlertCircle,
} from "lucide-react";
...
setAvatarPreview(mappedUser.avatar || "");
    setUser(parsedUser);
    setFormData({
    ...
    <AppLayout>
      <div className="px-4 py-4 max-w-lg mx-auto space-y-6 relative h-full">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-foreground">แก้ไขโปรไฟล์</h1>
        </div>
              ...
              <CardContent className="p-5 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <UserIcon
                      className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${fieldErrors.name ? "text-destructive" : "text-muted-foreground"}`}
                    />
                    <Input
                      name="name"
                      value={formData.name}
                      onChange={handleChange}
                      className={`pl-9 bg-background/50 font-medium transition-colors ${
                        fieldErrors.name
                ...
                {!isOldPasswordVerified ? (
                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <Lock
                          className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${fieldErrors.oldPassword ? "text-destructive" : "text-muted-foreground"}`}
                        />
                        <Input
                          ...
                          className={`pl-9 pr-10 bg-background/50 transition-colors ${
                  ...
                  <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                    <div className="flex items-center justify-between bg-green-500/10 text-green-600 px-3 py-2.5 rounded-xl text-xs font-bold border border-green-500/20 shadow-sm">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="h-4 w-4" /> รหัสผ่านเดิมถูกต้อง</div>
                      <button
                        type="button"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <Lock
                          className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${fieldErrors.newPassword ? "text-destructive" : "text-primary/70"}`}
                        />
                        <Input
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <Lock
                          className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${fieldErrors.confirmPassword ? "text-destructive" : "text-primary/70"}`}
                        />
                        <Input
        ...
        {toast.show && (
          <div className="fixed bottom-6 right-6 z-[100] flex items-start gap-3 w-72 p-4 rounded-2xl shadow-2xl border border-border/50 bg-background/80 backdrop-blur-xl animate-in slide-in-from-bottom-8 fade-in duration-300">
            <div
              className={`p-2 rounded-full shrink-0 ${toast.type === "success" ? "bg-green-500/10 text-green-600" : "bg-destructive/10 text-destructive"}`}
            >
              {toast.type === "success" ? (
