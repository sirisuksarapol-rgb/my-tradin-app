...
import { ArrowLeft, Star, ArrowRightLeft, Package, Flag, ShieldAlert, MessageSquare } from "lucide-react";
...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    const loadProfileAndRealDBStats = async () => {
      try {
        ...
        // 3. ผูกข้อมูลลงหน้าจอ
        const currentLoggedInID = String(loggedInUser?.MemberID || loggedInUser?.member_id || "");
        
        if (currentLoggedInID === String(userId)) {
          setUserData({
            MemberID: Number(userId),
            DisplayName: loggedInUser.DisplayName || loggedInUser.display_name || "ผู้ใช้งานทั่วไป",
            Email: loggedInUser.Email || loggedInUser.email || "ไม่มีข้อมูลอีเมล",
            ProfileImage: loggedInUser.ProfileImage || loggedInUser.profile_image || "",
          ...
          setUserData({
            MemberID: Number(userId),
            DisplayName: matchedPost?.DisplayName || stateData?.authorName || "ผู้ใช้งานทั่วไป",
            Email: matchedPost?.Email || stateData?.authorEmail || "ไม่มีข้อมูลอีเมล",
            ProfileImage: matchedPost?.ProfileImage || stateData?.rawProfileImg || "",
            totalItems: userItemsCount,
            successfulExchanges: dbExchanges,
            reviewScore: dbRating
          });
  ...
  if (rawImg && rawImg.trim() !== "undefined" && rawImg.trim() !== "null" && rawImg.trim() !== "") {
    profileImageUrl = rawImg.trim().startsWith('http') ? rawImg.trim() : `${IMAGE_BASE_URL}/uploads/${rawImg.trim()}`;
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
        ...
        <div className="grid md:grid-cols-[auto_1fr] gap-8 items-start">
          <Avatar className="h-28 w-28 mx-auto md:mx-0 border-4 border-background shadow-lg">
            {profileImageUrl && <AvatarImage src={profileImageUrl} alt={profileName} className="object-cover" />}
          ...
          <div className="space-y-3 text-center md:text-left">
            ...
            <div className="grid grid-cols-3 gap-4 pt-4 max-w-md mx-auto md:mx-0">
              {[
                { icon: Package, value: String(userData?.totalItems || 0), label: "รายการสิ่งของ" },
                { icon: ArrowRightLeft, value: String(userData?.successfulExchanges || 0), label: "แลกเปลี่ยนสำเร็จ" }, 
                { icon: Star, value: String(userData?.reviewScore || "0.0"), label: "คะแนนรีวิว" }
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
            <MessageSquare className="h-5 w-5 text-primary" /> คำวิจารณ์และรีวิวจากผู้ใช้งานจริง
          </h2>
          ...
          {reviews.length === 0 ? (
            <div className="text-center py-12 border border-dashed rounded-xl bg-muted/10 text-muted-foreground">
              ...
              <p className="text-sm">ผู้ใช้งานรายนี้ยังไม่ได้รับคำรีวิวความคิดเห็นในระบบ</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {reviews.map((review) => (
                <Card key={review.ExchangeID} className="bg-card/50 border shadow-sm">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-foreground">{review.ReviewerName || "ผู้ใช้งานทั่วไป"}</p>
        ...
        {fromAdmin && (
          <Button variant="secondary" onClick={() => navigate("/admin")} className="w-full max-w-xs gap-1.5 bg-primary/10 text-primary hover:bg-primary/20">
