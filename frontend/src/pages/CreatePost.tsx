          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                กรอกข้อมูลสิ่งของที่ต้องการนำมาแลกเปลี่ยน
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex justify-between items-center">
                  <Label
                    className={`text-sm font-bold ${errors.images ? "text-destructive" : ""}`}
                  >
                    รูปสินค้า (3-6 รูป){" "}
                    <span className="text-destructive">*</span>
                  </Label>
                  <span
                    className={`text-xs font-medium ${images.length < 3 ? "text-warning" : "text-success"}`}
                  >
                    {images.length === 0
                      ? "ยังไม่มีรูปภาพ"
                      : `${images.length}/6 รูป`}
                ...
                <div
                  className={`grid grid-cols-3 gap-3 p-3 rounded-2xl border ${errors.images ? "bg-destructive/5 border-destructive/20" : "border-border/50 bg-muted/20"}`}
                >
                  {images.map((img, i) => (
                    <div
                      key={i}
              ...
              <div className="lg:col-span-3 space-y-5">
                ...
                <div className="space-y-1.5">
                  <Label
                    htmlFor="title"
                    className={`text-sm font-bold ${errors.title ? "text-destructive" : ""}`}
                  >
                    ชื่อสิ่งของ <span className="text-destructive">*</span>
                  </Label>
                  <div className="relative">
                    <Package
                      className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${errors.title ? "text-destructive" : "text-primary"}`}
                    />
                    <Input
                      id="title"
                      placeholder="เช่น กล้องฟิล์ม Olympus Mju II"
                      value={title}
                      onChange={(e) => {
                ...
                <div className="space-y-1.5">
                  <Label
                    htmlFor="desc"
                    className={`text-sm font-bold ${errors.description ? "text-destructive" : ""}`}
                  >
                    รายละเอียด <span className="text-destructive">*</span>
                  </Label>
                  <div className="relative">
                    <AlignLeft
                      className={`absolute left-3 top-3 h-4 w-4 ${errors.description ? "text-destructive" : "text-primary"}`}
                    />
                    <Textarea
                      id="desc"
                      placeholder="บอกรายละเอียด สภาพ หรือตำหนิสินค้าของคุณ..."
                      value={description}
                      onChange={(e) => {
                ...
                <div className="space-y-1.5">
                  <Label
                    htmlFor="wanted"
                    className={`text-sm font-bold ${errors.wanted ? "text-destructive" : ""}`}
                  >
                    สิ่งที่ต้องการแลก{" "}
                    <span className="text-destructive">*</span>
                  </Label>
                  <div className="relative">
                    <ArrowRightLeft
                      className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${errors.wanted ? "text-destructive" : "text-primary"}`}
                    />
                    <Input
                      id="wanted"
                      placeholder="ระบุของที่คุณสนใจอยากแลกด้วย"
                      value={wanted}
                      onChange={(e) => {
                        setWanted(e.target.value);
                        if (errors.wanted)
                          setErrors((prev) => ({ ...prev, wanted: "" }));
                      }}
                      className={`pl-10 h-11 ${errors.wanted ? "border-destructive focus-visible:ring-destructive" : ""}`}
                    />
                  </div>
                  {errors.wanted && (
                    <p className="text-xs text-destructive">{errors.wanted}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label
                      htmlFor="location"
                      className={`text-sm font-bold ${errors.location ? "text-destructive" : ""}`}
                    >
                      สถานที่นัดรับ <span className="text-destructive">*</span>
                    </Label>
                    <div className="relative">
                      <MapPin
                        className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${errors.location ? "text-destructive" : "text-primary"}`}
                      />
                      <Input
                        id="location"
                        placeholder="เช่น เซ็นทรัล ลาดพร้าว"
                        value={location}
                        onChange={(e) => {
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="map-link" className="text-sm font-bold">
