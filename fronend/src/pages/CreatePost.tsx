...
import {
  ...
  Package, LayoutGrid, AlignLeft, ArrowRightLeft, Sparkles
} from "lucide-react";
...
export default function CreatePost() {
  ...
  const demo_title = "กล้อง Canon EOS M50";
  const demo_category = "อุปกรณ์อิเล็กทรอนิกส์และไอที";
  const demo_description = "กล้อง Mirrorless สภาพดี ใช้งานปกติ ถ่ายวิดีโอ 4K ได้";
  const demo_wanted = "iPad หรือ Tablet อื่นๆ";
  const demo_location = "เซ็นทรัล ลาดพร้าว / BTS ห้าแยกลาดพร้าว";
  const demo_mapLink = "https://maps.app.goo.gl/xxxxxx";
  ...
  const fillDemoData = () => {
    ...
    setWanted(demo_wanted);
    setLocation(demo_location);
    setMapLink(demo_mapLink);
    setErrors({});
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log({ images, title, category, description, wanted, location, mapLink });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
              ...
              <div className="lg:col-span-3 space-y-5">
                ...
                <div className="pt-4 flex flex-col sm:flex-row gap-3">
                  <Button type="submit" className="flex-1 h-12 text-base font-bold eco-gradient text-primary-foreground shadow-lg">
                    ลงประกาศแลกเปลี่ยน
                  </Button>
                  <Button type="button" variant="outline" className="sm:w-auto" onClick={() => navigate("/feed")}>
                    ยกเลิก
                  </Button>
                </div>
              </div>
